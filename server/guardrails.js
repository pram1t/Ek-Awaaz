...
export function sanitizeInput(raw) {
  ...
  for (const [re, replacement] of REDACTIONS) {
    if (re.test(text)) {
      redacted.push(replacement.replace(/\[|\]|removed/g, '').trim() || 'identifier');
